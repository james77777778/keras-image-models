...
keras_model_classes = [
    convmixer.ConvMixer736D32,
    convmixer.ConvMixer1024D20,
    convmixer.ConvMixer1536D20,
]
...
for timm_model_name, keras_model_class in zip(
    ...
    for keras_weight, keras_name in trainable_weights + non_trainable_weights:
        ...
        # stem
        torch_name = torch_name.replace("stem.conv2d", "stem.0")
        torch_name = torch_name.replace("stem.bn", "stem.2")
        # blocks
        torch_name = torch_name.replace("dwconv2d.", "")
        torch_name = torch_name.replace("conv2d.", "")
        # head
        torch_name = torch_name.replace("classifier", "head")
