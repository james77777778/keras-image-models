...
for timm_model_name, keras_model_class in zip(
    ...
    for keras_weight, keras_name in trainable_weights + non_trainable_weights:
        ...
        # stem
        torch_name = torch_name.replace("conv.stem.conv2d", "conv1")
        torch_name = torch_name.replace("conv.stem.bn", "bn1")
        # blocks
        torch_name = torch_name.replace("conv1.conv2d", "conv1")
        torch_name = torch_name.replace("conv1.bn", "bn1")
        torch_name = torch_name.replace("conv2.conv2d", "conv2")
        torch_name = torch_name.replace("conv2.bn", "bn2")
        torch_name = torch_name.replace("conv3.conv2d", "conv3")
