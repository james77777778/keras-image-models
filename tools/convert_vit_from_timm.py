...
keras_model_classes = [
    vision_transformer.VisionTransformerTiny16,
    vision_transformer.VisionTransformerSmall16,
    vision_transformer.VisionTransformerSmall32,
    vision_transformer.VisionTransformerBase16,
    vision_transformer.VisionTransformerBase32,
    vision_transformer.VisionTransformerLarge16,
    vision_transformer.VisionTransformerLarge32,
]
...
for timm_model_name, keras_model_class in zip(
    ...
    """
    input_shape = [384, 384, 3]  # use size of 384 for best performance
    ...
    for keras_weight, keras_name in trainable_weights + non_trainable_weights:
        ...
        # patch embedding
        torch_name = torch_name.replace("patch.embed.conv", "patch_embed.proj")
        # postition_embedding
        torch_name = torch_name.replace(
            "postition.embedding.pos.embed", "pos_embed"
        )
        torch_name = torch_name.replace(
            "postition.embedding.cls.token", "cls_token"
        )
        # blocks
    ...
    """
    export_path = f"exported/{keras_model.name.lower()}_imagenet_384.keras"
